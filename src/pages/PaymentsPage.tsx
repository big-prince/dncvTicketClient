...
const PaymentsPage: React.FC = () => {
  ...
  const handleExport = () => {
    if (filterStatus !== "approved") {
      toast.error("Export is only available for approved payments");
      return;
    }

    if (sortedPayments.length === 0) {
      toast.error("No approved payments to export");
      return;
    }
    ...
    let reportContent = `PAYMENT APPROVAL REPORT
Generated: ${reportDate}
Total Approved Payments: ${sortedPayments.length}
...
`;
    ...
    sortedPayments.forEach((payment, index) => {
      const paidDate = payment.paymentInfo.paidAt
        ? new Date(payment.paymentInfo.paidAt).toLocaleDateString("en-US", {
            year: "numeric",
            month: "short",
            day: "numeric",
          })
...
Ticket Details:
  Type: ${payment.ticketInfo.type}
  Quantity: ${payment.ticketInfo.quantity}
  Unit Price: ₦${payment.ticketInfo.unitPrice.toLocaleString()}
  Total Amount: ₦${payment.ticketInfo.totalAmount.toLocaleString()}

Payment Information:
  Method: ${payment.paymentInfo.method}
  Status: ${payment.paymentInfo.status}
  Paid Date: ${paidDate}
  Approved By: ${payment.paymentInfo.approvedBy || "N/A"}
...
SUMMARY
${"-".repeat(20)}
Total Revenue: ₦${sortedPayments
      .reduce((sum, p) => sum + p.ticketInfo.totalAmount, 0)
      .toLocaleString()}
Average Payment: ₦${Math.round(
      sortedPayments.reduce((sum, p) => sum + p.ticketInfo.totalAmount, 0) /
        sortedPayments.length
    ).toLocaleString()}

Report generated by DNCV Ticket System
...
`;
    ...
    link.setAttribute(
      "download",
      `approved_payments_report_${new Date().toISOString().split("T")[0]}.txt`
    );
    ...
    toast.success(`Approved payments report exported successfully`);
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-4 mt-4 md:mt-0">
              ...
              <div className="text-sm text-gray-400">
                ...
                {filterStatus === "pending" ? "pending" : "approved"} payments
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-colors"
            >
